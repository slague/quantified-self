...
function findMeal(id){
  return allMeals().then(function(data){
    return data[id-1]
  })
}

function addFoodToMeal(id){
  return findMeal(id).then(function(data){
    data.foods.push({name: foodName, calories: foodCalories})
  })
}


function allMeals(){
  return database.raw("SELECT * FROM meals").then(function(data){
    return data.rows
  }).then(function(meals){
